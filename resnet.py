...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, with_fc=True):
        self.inplanes = 64
...
def resnet34_feature(pretrained=False, num_classes=1000, **kwargs):
    ...
    rnet34.load_state_dict(torch.load('/home/nesa/zy/pretrained_model/resnet34-333f7ec4.pth'))
    '''
    pretrained_dict = rnet34.state_dict()
...
def resnet50_feature():
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    #model = ResNet(Bottleneck, [3, 4, 6, 3],with_fc=False)
    ...
    '''
    pretrained_dict = rnet50.state_dict()
    model_dict = model.state_dict()
    # 将pretrained_dict里不属于model_dict的键剔除掉
    pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}
    # 更新现有的model_dict
...
def resnet101_feature():
    ...
    rnet101.load_state_dict(torch.load('/home/nesa/zy/pretrained_model/resnet101-5d3b4d8f.pth'))
    '''
    pretrained_dict = rnet101.state_dict()
    model_dict = model.state_dict()
    # 将pretrained_dict里不属于model_dict的键剔除掉
    pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}
    # 更新现有的model_dict
